...
export default function splash({ path, name, org, coc, logo, active, total, channels, iframe, username }){
  let div = dom('.splash',
    !iframe && dom('.logos',
      logo && dom('.logo.org'),
      dom('.logo.slack')
    ...
    dom('form',
      ...
      dom(`input.form-item type=hidden placeholder=you@yourdomain.com value=${username}@sfu.ca`),
      coc && dom('.coc',
        dom('label',
      ...
      dom('button.loading', `Send Invite to ${username}@sfu.ca`)
    ),
    !iframe && dom('p.signin',
      'or ',
