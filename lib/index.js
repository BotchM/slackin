...
const casauth = cas.getMiddleware({
...
var loggedin = function(req, res, next) {
    ...
    const redirectUrl = req.query.ticket ? `/login?ticket=${req.query.ticket}` : '/login'
...
export default function slackin({
  ...
  app.use(session({
    ...
    secret: process.env.SESSION_SECRET || 'ssssssh!',
  }));
  ...
  app.get('/', loggedin, (req, res) => {
    ...
    let username = req.session.auth.user;
    ...
    let page = dom('html',
      ...
      splash({ coc, path, css, name, org, logo, channels, active, total, username })
    );
    res.type('html');
  ...
  // invite endpoint
  app.post('/invite', loggedin, json(), (req, res, next) => {
    let chanId;
    if (channels) {
